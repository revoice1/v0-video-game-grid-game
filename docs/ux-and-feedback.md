...
- Daily guess inserts and objection persistence now go through a server-only privileged write path, so public guess-row inserts or updates are not required in Supabase RLS.
